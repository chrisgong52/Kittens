...
CARD_BUTTONS = {"exploding kitten": Button(root, text = "exploding kitten", command = lambda:on_click("add", "exploding kitten")),
                "diffuse": Button(root, text = "diffuse", command = lambda:on_click("add", "diffuse")),
                "tacocat": Button(root, text = "tacocat", command = lambda:on_click("add", "tacocat")),
                "beard cat": Button(root, text = "beard cat", command = lambda:on_click("add", "beard cat")),
                "cattermelon": Button(root, text = "cattermelon", command = lambda:on_click("add", "cattermelon")),
                "attack": Button(root, text = "attack", command = lambda:on_click("add", "attack")),
                "rainbow-ralphing cat": Button(root, text = "rainbow-ralphing cat", command = lambda:on_click("add", "rainbow-ralphing cat")),
                "shuffle": Button(root, text = "shuffle", command = lambda:on_click("add", "shuffle")),
                "favor": Button(root, text = "favor", command = lambda:on_click("add", "favor")),
                "skip": Button(root, text = "skip", command = lambda:on_click("add", "skip")),
                "see the future": Button(root, text = "see the future", command = lambda:on_click("add", "see the future")),
                "nope": Button(root, text = "nope", command = lambda:on_click("add", "nope")),
                "hairy potato cat": Button(root, text = "hairy potato cat", command = lambda:on_click("add", "hairy potato cat"))
                }
...
#btn1 = Button(root, text = "Add Card", command = lambda:on_click("add", "beard cat"))
...
while not EXIT:
    ...
    for card in PLAYERS[0].hand:
        ...
        left_bound = left_bound + width-20
    canvas.update()
count = 0
