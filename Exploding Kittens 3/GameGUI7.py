...
def on_click(action: string, name: string):
    ...
    if action == "push":
        ...
        ####
        ####    TURN THIS INTO CALLING STATETESTS play_card
        ####
        global ACTION_CARDS
        global DECK
        if len(name) != 0 and StateTests.check_valid(name, ACTION_CARDS) != "invalid":
            ...
            if len(name) == 1:
                ...
                DISCARD.push("match 2")
            elif len(name) == 3:
                DISCARD.push("match 3")
            else:
                DISCARD.push("different")
            ind = len(PLAYERS[CURRENT_PLAYER[0]].hand) - 1
            while(ind > -1):
                if PLAYERS[CURRENT_PLAYER[0]].is_selected(ind):
                    PLAYERS[CURRENT_PLAYER[0]].remove(ind)
                ind = ind - 1
                
            while(not PLAYERS[CURRENT_PLAYER[0]].play_empty):
                PLAYERS[CURRENT_PLAYER[0]].play.pop(0)
            print("discard: ", end = "")
            DISCARD.print()'''
            action_list = []
